...
const WIDGET_BASE_ID = 'pgthrottle-cat';
...
function admin_scripts($hook) {
	...
	if ($hook == 'widgets.php') { // enqueue only for widget admin and customizer
		...
		wp_localize_script( 'category-posts-widget-admin-js', 'cwp_default_thumb_selection', array(
			'frame_title' => __( 'Select a default thumbnail', 'pgthrottle' ),
			'button_title' => __( 'Select', 'pgthrottle' ),
			'none' => __( 'None', 'pgthrottle' ),
		) );
	}	
}
...
function load_textdomain() {
  load_plugin_textdomain( 'pgthrottle', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' ); 
}
...
class Widget extends \WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'cat-post-widget', 'description' => __('List single category posts','pgthrottle'));
		parent::__construct(WIDGET_BASE_ID, __('Category Posts','pgthrottle'), $widget_ops);
	}
		...
		if( !isset($instance["title"]) || !$instance["title"] ) {
            ...
            if (isset($instance["cat"])) {
                ...
                if ($category_info && !is_wp_error($category_info))
					...
					$instance["title"] = __('Recent Posts','pgthrottle');
            } else 
					$instance["title"] = __('Recent Posts','pgthrottle');
		} 
...
?>    
        <h4 data-panel="title"><?php _e('Title','pgthrottle')?></h4>
        <div>
            <p>
                <label for="<?php echo $this->get_field_id("title"); ?>">
                    <?php _e( 'Title','pgthrottle' ); ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id("title_link"); ?>">
                    ...
                    <?php _e( 'Make widget title link','pgthrottle' ); ?>
                </label>
            </p>
            <p>
...
?>
        <h4 data-panel="filter"><?php _e('Filter','pgthrottle');?></h4>
        <div>
            <p>
                <label>
                    <?php _e( 'Category','pgthrottle' ); ?>:
                    <?php wp_dropdown_categories( array( 'show_option_all' => __('All categories','pgthrottle'), 'hide_empty'=> 0, 'name' => $this->get_field_name("cat"), 'selected' => $instance["cat"] ) ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("no_cat_childs"); ?>">
                    ...
                    <?php _e( 'Exclude child categories','pgthrottle' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("num"); ?>">
                    <?php _e('Number of posts to show','pgthrottle'); ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id("offset"); ?>">
                    <?php _e('Start offsett','pgthrottle'); ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id("sort_by"); ?>">
                    <?php _e('Sort by','pgthrottle'); ?>:
                    <select id="<?php echo $this->get_field_id("sort_by"); ?>" name="<?php echo $this->get_field_name("sort_by"); ?>">
                        <option value="date"<?php selected( $instance["sort_by"], "date" ); ?>><?php _e('Date','pgthrottle')?></option>
                        <option value="title"<?php selected( $instance["sort_by"], "title" ); ?>><?php _e('Title','pgthrottle')?></option>
                        <option value="comment_count"<?php selected( $instance["sort_by"], "comment_count" ); ?>><?php _e('Number of comments','pgthrottle')?></option>
                        <option value="rand"<?php selected( $instance["sort_by"], "rand" ); ?>><?php _e('Random','pgthrottle')?></option>
                    </select>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("asc_sort_order"); ?>">
                    <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("asc_sort_order"); ?>" name="<?php echo $this->get_field_name("asc_sort_order"); ?>" <?php checked( (bool) $instance["asc_sort_order"], true ); ?> />
                            <?php _e( 'Reverse sort order (ascending)','pgthrottle' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("exclude_current_post"); ?>">
                    ...
                    <?php _e( 'Exclude current post','pgthrottle' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("hideNoThumb"); ?>">
                    ...
                    <?php _e( 'Exclude posts which have no thumbnail','pgthrottle' ); ?>
                </label>
            </p>					
        </div>			
<?php
...
?>        
        <h4 data-panel="thumbnail"><?php _e('Thumbnails','pgthrottle')?></h4>
        <div>
            <p>
                <label for="<?php echo $this->get_field_id("thumb"); ?>">
                    <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("thumb"); ?>" name="<?php echo $this->get_field_name("thumb"); ?>"<?php checked( (bool) $instance["thumb"], true ); ?> />
                    <?php _e( 'Show post thumbnail','pgthrottle' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("thumbTop"); ?>">
                    ...
                    <?php _e( 'Show thumbnails above text','pgthrottle' ); ?>
                </label>
            </p>
            <p>
                <label>
                    <?php _e('Thumbnail dimensions (in pixels)','pgthrottle'); ?>:<br />
                    <label for="<?php echo $this->get_field_id("thumb_w"); ?>">
                        <?php _e('Width:','pgthrottle')?> <input class="widefat" style="width:30%;" type="number" min="1" id="<?php echo $this->get_field_id("thumb_w"); ?>" name="<?php echo $this->get_field_name("thumb_w"); ?>" value="<?php echo esc_attr($instance["thumb_w"]); ?>" />
                    </label>
                    
                    <label for="<?php echo $this->get_field_id("thumb_h"); ?>">
                        <?php _e('Height:','pgthrottle')?> <input class="widefat" style="width:30%;" type="number" min="1" id="<?php echo $this->get_field_id("thumb_h"); ?>" name="<?php echo $this->get_field_name("thumb_h"); ?>" value="<?php echo esc_attr($instance["thumb_h"]); ?>" />
                    </label>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("use_css_cropping"); ?>">
                    ...
                    <?php _e( 'CSS crop to requested size ','pgthrottle' ); ?>
                </label>
            </p>					
            <p>
                <label style="display:block">
                    <?php _e( 'Default thumbnail ','pgthrottle' ); ?>
                </label>
			...
			<p>
				<button type="button" class="cwp_default_thumb_select button upload-button">
					<?php _e('Select image','pgthrottle')?>
				</button>
				<button type="button" class="cwp_default_thumb_remove button upload-button" <?php if (!$default_thunmbnail) echo 'style="display:none"' ?> >
					<?php _e('No default','pgthrottle')?>
				</button>
            </p>					
            <p>
                <label for="<?php echo $this->get_field_id("thumb_hover"); ?>">
                    <?php _e( 'Animation on mouse hover:','pgthrottle' ); ?>
                </label>
                <select id="<?php echo $this->get_field_id("thumb_hover"); ?>" name="<?php echo $this->get_field_name("thumb_hover"); ?>">
                    <option value="none" <?php selected($thumb_hover, 'none')?>><?php _e( 'None', 'pgthrottle' ); ?></option>
                    <option value="dark" <?php selected($thumb_hover, 'dark')?>><?php _e( 'Darker', 'pgthrottle' ); ?></option>
                    <option value="white" <?php selected($thumb_hover, 'white')?>><?php _e( 'Brighter', 'pgthrottle' ); ?></option>
                    <option value="scale" <?php selected($thumb_hover, 'scale')?>><?php _e( 'Zoom in', 'pgthrottle' ); ?></option>
					<option value="blur" <?php selected($thumb_hover, 'blur')?>><?php _e( 'Blur', 'pgthrottle' ); ?></option>
                </select>
            </p>
        </div>
<?php
	...
	function form($instance) {
		...
		<div class="category-widget-cont">
            <p><a target="_blank" href="http://tiptoppress.com/term-and-category-based-posts-widget/?utm_source=widget_cpw&utm_campaign=get_pro_cpw&utm_medium=form"><?php _e('Get the Pro version','pgthrottle'); ?></a></p>
        <?php
            $this->formTitlePanel($instance);
            $this->formFilterPanel($instance);
            $this->formThumbnailPanel($instance);
        ?>
			<h4 data-panel="details"><?php _e('Post details','pgthrottle')?></h4>
			<div>
				<p>
					<label for="<?php echo $this->get_field_id("everything_is_link"); ?>">
						...
						<?php _e( 'Everything is a link','pgthrottle' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("hide_post_titles"); ?>">
						...
						<?php _e( 'Hide post titles','pgthrottle' ); ?>
					</label>
				</p>
				<p>
				...
				<div class="cpwp_ident categoryposts-data-panel-excerpt" style="display:<?php echo ((bool) $excerpt) ? 'block' : 'none'?>">
					...
					<p>
						<label for="<?php echo $this->get_field_id("excerpt_more_text"); ?>">
							<?php _e( 'Excerpt \'more\' text:','pgthrottle' ); ?>
						</label>
						<input class="widefat" style="width:45%;" placeholder="<?php _e('... more','pgthrottle')?>" id="<?php echo $this->get_field_id("excerpt_more_text"); ?>" name="<?php echo $this->get_field_name("excerpt_more_text"); ?>" type="text" value="<?php echo esc_attr($instance["excerpt_more_text"]); ?>" />
					</p>
					<p>
						<label for="<?php echo $this->get_field_id("excerpt_filters"); ?>">
							...
							<?php _e( 'Themes and plugins may override','pgthrottle' ); ?>
						</label>
					</p>
				</div>
				<p>
					<label for="<?php echo $this->get_field_id("date"); ?>" onchange="javascript:cwp_namespace.toggleDatePanel(this)">
						...
						<?php _e( 'Show post date','pgthrottle' ); ?>
					</label>
				</p>
				<div class="cpwp_ident categoryposts-data-panel-date" style="display:<?php echo ((bool) $date) ? 'block' : 'none'?>">
					...
					<p>
						<label for="<?php echo $this->get_field_id("date_link"); ?>">
							...
							<?php _e( 'Make widget date link','pgthrottle' ); ?>
						</label>
					</p>
				</div>
				<p>
					<label for="<?php echo $this->get_field_id("comment_num"); ?>">
						...
						<?php _e( 'Show number of comments','pgthrottle' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("author"); ?>">
						...
						<?php _e( 'Show post author','pgthrottle' ); ?>
					</label>
				</p>
			</div>
			<h4 data-panel="general"><?php _e('General','pgthrottle')?></h4>
			<div>
				<p>
					<label for="<?php echo $this->get_field_id("disable_css"); ?>">
						...
						<?php _e( 'Disable the built-in CSS for this widget','pgthrottle' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("hide_if_empty"); ?>">
						...
						<?php _e( 'Hide widget if there are no matching posts','pgthrottle' ); ?>
					</label>
				</p>
			</div>
			<h4 data-panel="footer"><?php _e('Footer','pgthrottle')?></h4>
			<div>
				<p>
					<label for="<?php echo $this->get_field_id("footer_link"); ?>">
						<?php _e( 'Footer link text','pgthrottle' ); ?>:
						<input class="widefat" style="width:60%;" placeholder="<?php _e('... more by this topic','pgthrottle')?>" id="<?php echo $this->get_field_id("footer_link"); ?>" name="<?php echo $this->get_field_name("footer_link"); ?>" type="text" value="<?php echo esc_attr($instance["footer_link"]); ?>" />
					</label>
				</p>
			</div>
            <p><a href="<?php echo get_edit_user_link().'#'.__NAMESPACE__ ?>"><?php _e('Widget admin behaviour settings','pgthrottle')?></a></p>			
            <p><a target="_blank" href="<?php echo CAT_POST_DOC_URL ?>"><?php _e('Documentation','pgthrottle'); ?></a></p>
            <p><?php echo sprintf( wp_kses( __( 'We are on <a href="%1$s">Facebook</a> and <a href="%2$s">Twitter</a>.', 'pgthrottle' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( 'https://www.facebook.com/TipTopPress' ), esc_url( 'https://twitter.com/TipTopPress' ) ); ?></br></br></p>
		</div>
		<?php
	}
}
...
function add_action_links ( $links ) {
    $pro_link = array(
        '<a target="_blank" href="http://tiptoppress.com/term-and-category-based-posts-widget/?utm_source=widget_cpw&utm_campaign=get_pro_cpw&utm_medium=action_link">'.__('Get the Pro version','pgthrottle').'</a>',
    );
...
function customize_register($wp_customize) {
    ...
    if (count($posts) > 0) {
        $wp_customize->add_panel( __NAMESPACE__, array(
            'title'           => __( 'Category Posts Shortcode', 'pgthrottle' ),
            'priority'        => 300,
        ...
        foreach($posts as $p) {
			...
			foreach ($meta as $k => $m) {
				...
				$section_title = $k;
				if ($section_title == '')
					$section_title = __('[shortcode]', 'pgthrottle');
...
function mce_external_languages($locales) {
	if (current_user_can('edit_theme_options'))  // don't load the code if the user can not customize the shortcode
		...
		if (is_array($meta) && isset($meta['editor']))
			...
			$locales['pgthrottle'] = plugin_dir_path ( __FILE__ ) . 'tinymce_translations.php';
    return $locales;
}
...
function show_user_profile( $user ) { 
	...
	if (isset($meta['editor']))
		$editor = true;
?>
	<h3 id="<?php echo __NAMESPACE__ ?>"><?php _e('Category Posts Widget behaviour settings','pgthrottle')?></h3>

	<table class="form-table">
		<tr>
			<th><label for="<?php echo __NAMESPACE__?>[panels]"><?php _e('Open panels behavior','pgthrottle')?></label></th>
			<td>
				<input type="checkbox" name="<?php echo __NAMESPACE__?>[panels]" id="<?php echo __NAMESPACE__?>[panels]" <?php checked($accordion); ?>">
				<label for=<?php echo __NAMESPACE__?>[panels]><?php _e('Close the curremtly open panel when opening a new one','pgthrottle')?></label>
			</td>
		</tr>
		<tr>
			<th><label for="<?php echo __NAMESPACE__?>[editor]"><?php _e('Visual editor button','pgthrottle')?></label></th>
			<td>
				<input type="checkbox" name="<?php echo __NAMESPACE__?>[editor]" id="<?php echo __NAMESPACE__?>[editor]" <?php checked($editor); ?>">
				<label for="<?php echo __NAMESPACE__?>[editor]"><?php _e('Hide the "insert shortcode" button from the editor','pgthrottle')?></label>
			</td>
		</tr>
	</table>
<?php 
